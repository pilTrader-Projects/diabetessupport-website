const fs = require('fs');
const path = require('path');

/**
 * Executes system environment and critical asset sanity checks before code delivery.
 *
 * @usecase Verifies all mandatory documentation, standards, configuration files, environment templates, docker compose file, and core application files exist.
 * @param None Reads process.cwd() file system tree.
 * @dependencies fs, path modules.
 * @returns {Promise<void>} Resolves when all required file paths exist.
 * @throws {Error} Throws error with file path details if any required file is missing.
 */
async function runSanityCheck() {
    console.log("🚀 Starting Environment Sanity Check...");

    const requiredPaths = [
        'docs/standards.md',
        'docs/guardrails.md',
        'docs/intended-use-statement.md',
        'docs/brand_repositioning_rebranding_strategy.md',
        '.agent/personas.md',
        '.agent/workflows/coo_orchestration_flow.md',
        '.agent/workflows/cross_functional_campaign_flow.md',
        '.agent/workflows/seo_geo_optimization_flow.md',
        '.agents/rules/company_operating_system.md',
        'ROADMAP.md',
        '.env.example',
        'docker-compose.yml',
        'src/config/constants.ts',
        'src/config/leadConfig.ts',
        'src/lib/dbConnect.ts',
        'src/lib/schema.ts',
        'src/lib/communityUtils.ts',
        'src/lib/communityNotifications.ts',
        'src/types/blog.ts',
        'src/types/community.ts',
        'src/types/learning.ts',
        'src/models/Post.ts',
        'src/models/Category.ts',
        'src/models/ApiKey.ts',
        'src/models/Thread.ts',
        'src/models/Reply.ts',
        'src/models/DeviceSync.ts',
        'src/models/Lead.ts',
        'src/models/CampaignConfig.ts',
        'src/models/AppConfig.ts',
        'src/models/AssetDownloadToken.ts',
        'src/models/Authority.ts',
        'src/models/LearningResource.ts',
        'src/services/wordpressMigration.ts',
        'src/services/brevoService.ts',
        'src/services/campaignService.ts',
        'src/services/appConfigService.ts',
        'src/services/assetStorageService.ts',
        'src/services/learningService.ts',
        'src/services/aiQualifierService.ts',
        'scripts/migrate-wordpress.ts',
        'src/app/api/v1/migration/route.ts',
        'src/app/api/v1/posts/route.ts',
        'src/app/api/v1/subscribe/route.ts',
        'src/app/api/v1/leads/route.ts',
        'src/app/api/v1/app-config/route.ts',
        'src/app/api/v1/admin/login/route.ts',
        'src/app/api/v1/admin/campaigns/route.ts',
        'src/app/api/v1/admin/assets/route.ts',
        'src/app/api/v1/admin/app-config/route.ts',
        'src/app/api/v1/admin/learning/authorities/route.ts',
        'src/app/api/v1/admin/learning/sync/route.ts',
        'src/app/api/v1/admin/learning/validate/route.ts',
        'src/app/api/v1/admin/learning/resources/route.ts',
        'src/app/api/v1/cron/learning-sync/route.ts',
        'src/app/api/v1/learning/resources/route.ts',
        'src/app/api/v1/learning/authorities/route.ts',
        'src/app/api/v1/assets/download/route.ts',
        'src/app/api/v1/community/threads/route.ts',
        'src/app/api/v1/community/threads/[slug]/replies/route.ts',
        'src/app/api/v1/community/threads/[slug]/like/route.ts',
        'src/app/api/v1/community/sync/route.ts',
        'src/lib/auth.ts',
        'src/lib/adminAuth.ts',
        'src/app/robots.ts',
        'src/app/ads.txt/route.ts',
        'src/app/sitemap.xml/route.ts',
        'src/app/feed.xml/route.ts',
        'src/components/ExtensionGuard.tsx',
        'src/components/Header.tsx',
        'src/components/NewsletterOptInForm.tsx',
        'src/components/KitOptInForm.tsx',
        'src/components/LeadMagnetCard.tsx',
        'src/components/ads/AdSenseScript.tsx',
        'src/components/ads/AdUnit.tsx',
        'src/components/community/CommunityHeader.tsx',
        'src/components/community/ThreadCard.tsx',
        'src/components/community/ReplyCard.tsx',
        'src/components/community/ReplyForm.tsx',
        'src/components/community/SocialShareBar.tsx',
        'src/components/community/CreateThreadModal.tsx',
        'src/components/community/SyncDeviceModal.tsx',
        'src/components/community/CommunityFeedClient.tsx',
        'src/components/community/ThreadViewClient.tsx',
        'src/app/community/page.tsx',
        'src/app/community/[slug]/page.tsx',
        'src/components/home/HeroSection.tsx',
        'src/components/home/CoreMetricsSection.tsx',
        'src/components/home/ManualAdvantageSection.tsx',
        'src/components/home/RoadmapsSection.tsx',
        'src/components/home/MealPlanModal.tsx',
        'src/components/funnel/SymptomChecklist.tsx',
        'src/components/funnel/LeadCaptureForm.tsx',
        'src/components/funnel/InsulinResetClient.tsx',
        'src/app/insulin-reset/page.tsx',
        'src/app/hidden-clock/page.tsx',
        'src/app/reset-success/page.tsx',
        'src/app/glycosense/page.tsx',
        'src/app/subscribe/page.tsx',
        'src/app/guides/cheatsheet/page.tsx',
        'src/app/sync/page.tsx',
        'src/app/privacy-policy/page.tsx',
        'src/app/terms-of-service/page.tsx',
        'src/app/about/page.tsx',
        'src/app/about/story/page.tsx',
        'src/app/contact/page.tsx',
        'src/app/not-found.tsx',
        'src/app/admin/page.tsx',
        'src/app/admin/login/page.tsx',
        'src/app/admin/campaigns/page.tsx',
        'src/app/admin/posts/page.tsx',
        'src/app/admin/assets/page.tsx',
        'src/app/admin/app-settings/page.tsx',
        'src/app/admin/learning/authorities/page.tsx',
        'src/app/admin/learning/resources/page.tsx',
        'src/components/admin/CampaignsManagerClient.tsx',
        'src/components/admin/CampaignModal.tsx',
        'src/components/admin/AssetsManagerClient.tsx',
        'src/components/admin/AppSettingsManagerClient.tsx',
        'src/components/admin/learning/AuthorityModal.tsx',
        'src/components/admin/learning/AuthoritiesManagerClient.tsx',
        'src/components/admin/learning/ResourceModal.tsx',
        'src/components/admin/learning/ResourcesManagerClient.tsx',
        'src/components/learning/VideoPlayerModal.tsx',
        'src/components/learning/SavedResourcesDrawer.tsx',
        'src/components/learning/LearningHubClient.tsx',
        'src/app/blog/page.tsx',
        'src/app/manifest.ts',
        'src/app/offline/page.tsx',
        'src/components/pwa/PwaRegister.tsx',
        'public/manifest.json',
        'public/sw.js',
        'public/icons/icon-192x192.png',
        'public/icons/icon-512x512.png',
        'public/icons/icon-maskable-192x192.png',
        'public/icons/icon-maskable-512x512.png',
        'src/app/layout.tsx',
        'src/app/template.tsx',
        'src/app/page.tsx',
        'src/app/globals.css'
    ];

    console.log("Checking required files...");
    for (const relPath of requiredPaths) {
        const fullPath = path.join(process.cwd(), relPath);
        if (!fs.existsSync(fullPath)) {
            throw new Error(`❌ Missing required file: ${relPath}`);
        }
        console.log(`✅ Verified: ${relPath}`);
    }

    console.log("\n✨ Sanity check passed! The environment is ready.");
}

runSanityCheck().catch((err: Error) => {
    console.error(err.message);
    process.exit(1);
});
