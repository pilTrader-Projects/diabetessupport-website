 ...
 *
 * @usecase Generates search engine compliant XML sitemap dynamically from active database articles, community threads, and static routes.
 */
export async function GET(): Promise<NextResponse> {
  ...
  try {
    await dbConnect();
    [posts, threads] = await Promise.all([
  ...
  const allRoutes = [...staticRoutes, ...postRoutes, ...threadRoutes];
